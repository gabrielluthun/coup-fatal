...
export function GameSetup() {
  ...
  const duration = useDurationField();

  const namesValid = player1Name.trim() !== '' && player2Name.trim() !== '';
  const canStart = namesValid && duration.isValid;

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!canStart) return;
    startGame(player1Name.trim(), player2Name.trim(), duration.durationMs);
  }
    ...
    >
      ...
      <div className="flex flex-col gap-5 sm:gap-7 w-full max-w-xl bg-zinc-950 border-zinc-800/60 rounded-3xl sm:rounded-4xl p-6 sm:p-8">
        ...
        <DurationSelector
          ...
          onSelectPreset={duration.selectPreset}
          isCustom={duration.isCustom}
          onSelectCustom={duration.selectCustom}
          customSeconds={duration.customSeconds}
          onCustomSecondsChange={duration.setCustomSeconds}
        />
      </div>
